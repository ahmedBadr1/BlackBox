...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <a href="{{route('admin.zones.index')}}">{{__("names.manage")}} {{__("names.zones")}}</a>
                ...
                <div class="d-flex ">
                    @can('zone-edit')
                        <a href="{{ route('admin.zones.edit',$zone->id) }}" class="btn btn-info o">{{__("auth.edit")}}</a>
                    @endcan
                    @can('zone-delete')
                        <form class="ml-5" action="{{route('admin.ones.destroy',$zone->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
