    ...
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <p><b> {{$order->address}}, <a href="{{route('admin.areas.show',$order->area->id )}}">{{ $order->area->name }}</a>, {{$order->state}}</b></p> <hr>
                ...
                <div class="d-flex ">
                    <a href="{{ route('admin.orders.edit',$order->id) }}" class="btn btn-info o">{{__("auth.edit")}}</a>
                    <form class="ml-5" action="{{route('admin.orders.destroy',$order->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
